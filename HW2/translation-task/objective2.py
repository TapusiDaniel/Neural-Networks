...
class EncoderRNN(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        h_n = h_n.transpose(0, 1).contiguous().view(1, -1, self.hidden_size)
        c_n = c_n.transpose(0, 1).contiguous().view(1, -1, self.hidden_size)
...
def plot_comparison_metrics(test_name, configs, all_metrics):
    ...
    plt.ylabel('Loss')
    plt.grid(True)
    plt.savefig(f'Results/best_training_loss_{test_name}.png', bbox_inches='tight')
    plt.close()

    # Validation Loss
    plt.figure(figsize=(10, 6))
    plt.plot(metrics['valid_losses'])
    plt.title('Validation Loss')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.grid(True)
    plt.savefig(f'Results/best_validation_loss_{test_name}.png', bbox_inches='tight')
    plt.close()

    # BLEU Score
    plt.figure(figsize=(10, 6))
    plt.plot(metrics['valid_bleu_scores'])
    plt.title('BLEU Score')
    plt.xlabel('Epoch')
    plt.ylabel('BLEU Score')
    plt.grid(True)
    plt.savefig(f'Results/best_bleu_score_{test_name}.png', bbox_inches='tight')
    plt.close()

    # Perplexity
    plt.figure(figsize=(10, 6))
    plt.plot(metrics['valid_perplexities'])
    plt.title('Perplexity')
    plt.xlabel('Epoch')
    plt.ylabel('Perplexity')
    plt.grid(True)
    plt.savefig(f'Results/best_perplexity_{test_name}.png', bbox_inches='tight')
    plt.close()
